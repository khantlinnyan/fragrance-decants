...
export function ProductCard({ fragrance }: ProductCardProps) {
  ...
  return (
    <Link to={`/product/${fragrance.id}`} className="group block">
      <div className="bg-neutral-900 rounded-lg overflow-hidden transition-all duration-300 hover:bg-neutral-800">
        {/* Image placeholder */}
        ...
        <div className="p-6">
          <div className="mb-2">
            ...
            <h3 className="text-lg font-light text-white group-hover:text-neutral-200 transition-colors">
              {fragrance.name}
            </h3>
          </div>
          ...
          <div className="mb-4">
            <p className="text-xs text-neutral-400 mb-1">Scent Family</p>
            <p className="text-sm text-neutral-300">{fragrance.scent_family}</p>
          </div>
          
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-neutral-400">From</p>
              <p className="text-lg font-light text-white">${minPrice.toFixed(2)}</p>
            </div>
